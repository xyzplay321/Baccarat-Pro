...
class BaccaratPro:
    ...
    def calculate_kelly_units(self, p_win, is_banker):
        ...
        f = (b * p - q) / b
        return min(round(f * 100, 1), 10) if f > 0 else 0
    ...
    def _finalize_exact_cards(self, vals):
        ...
        self.history_log.insert(0, f"輸入 [{valid_cards_str}] -> 閒 {final_p} 點 vs 莊 {final_b} 點 ({'莊贏' if res=='B' else '閒贏' if res=='P' else '和局'})")
        self.round_num += 1
        return True
    ...
    def process_blind_shortcut(self, cmd):
        ...
        self.raw_road.append(res); self._add_to_big_road(res)
        self.history_log.insert(0, f"快捷鍵 [{res}] -> {'莊贏' if res=='B' else '閒贏' if res=='P' else '和局'}")
        self.round_num += 1
        return True
...
def handle_command():
    ...
    if cmd == 'R': game.reset_game()
        ...
        else: game.history_log.insert(0, "⚠️ 點數長度錯誤")
    ...
    # 構建終端機文字畫面 (Terminal UI Generator)
    tw = {'B': '莊', 'P': '閒', 'T': '和'}
    lines = []
    lines.append("=============================================")
    lines.append(f"【 第 {game.round_num} 局 】" + (f"  🔥 連勝: {game.session_streak}" if game.session_streak > 0 else ""))
    lines.append("---------------------------------------------")
    ...
    diff = big_removed - small_removed # 消耗的大牌比小牌多，代表牌庫剩小牌
    ...
    lines.append("")
    road_sig = f"🔴 指向【莊】 (莊 {v_b} 票 vs 閒 {v_p} 票)" if v_b > v_p else f"🔵 指向【閒】 (閒 {v_p} 票 vs 莊 {v_b} 票)" if v_p > v_b else "⚪ 訊號分歧"
